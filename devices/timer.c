...
static struct list lista_dormindo;

static bool
despertar_menor (const struct list_elem *a, const struct list_elem *b,
                 void *aux UNUSED)
{
  const struct thread *ta = list_entry (a, struct thread, elem);
  const struct thread *tb = list_entry (b, struct thread, elem);

  if (ta->tick_despertar != tb->tick_despertar)
    return ta->tick_despertar < tb->tick_despertar;
